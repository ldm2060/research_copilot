...
```text
.copilot/pipelines/YYYY-MM-DD-S7-copilot-rebuttal-round-N.md
```
...
Worker returns MUST be reduced into the ledger before they influence stage output. Accepted responses and follow-up needs are written to `rebuttal/round-N.md` or `.copilot/handoff.md` only after your coordinator review verifies evidence, word budget, and cross-reviewer consistency.
...
## Rebuttal worker patterns

- Comment workers may classify one reviewer comment against existing evidence.
- Evidence workers may verify whether a response can cite a section, table, figure, or experiment run.
- Draft workers may write one bounded response under the active word-budget strategy.
- Consistency workers may compare numbers, promises, and claims across reviewer responses.
- You still own final tone, word count, cross-reviewer consistency, and follow-up routing signals.
