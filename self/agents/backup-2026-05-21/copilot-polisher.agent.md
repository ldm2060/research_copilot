...
```text
.copilot/pipelines/YYYY-MM-DD-S5-copilot-polisher-round-N.md
```
...
Worker returns MUST be reduced into the ledger before they influence stage output. Accepted edits are written to target tex files or `.copilot/handoff.md` only after your coordinator review verifies meaning preservation, LaTeX safety, and conflicts.
...
## Polishing worker patterns

- De-AI workers may rewrite bounded paragraphs without changing technical claims.
- Terminology workers may check that the same concept uses one English term.
- Syntax workers may improve sentence density while preserving LaTeX commands.
- You still own the final meaning-preservation check before editing files.
