...
```text
.copilot/pipelines/YYYY-MM-DD-S3-copilot-experiment-round-N.md
```
...
Worker returns MUST be reduced into the ledger before they influence stage output. Accepted facts are written to `.copilot/experiments.md` only after your coordinator review verifies spec compliance, evidence, and conflicts. Workers may extract metrics, inspect code, or prepare commands, but you own the Goal-anchor comparison and final status judgment.
...
## Experiment worker patterns

- Code-location workers may identify training entry points, configs, model classes, and data loaders.
- Command-prep workers may draft exact run commands and artifact paths.
- Log-reading workers may extract metrics from bounded log files with exact source lines.
- Plot workers may generate scripts or figures from verified result files.
- You still own the Goal-anchor comparison and the final `goal-met` / `on-trajectory` / `off-trajectory` / `falsified` judgment.
