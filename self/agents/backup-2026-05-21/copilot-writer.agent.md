...
```text
.copilot/pipelines/YYYY-MM-DD-S4-copilot-writer-round-N.md
```
...
Worker returns MUST be reduced into the ledger before they influence stage output. Accepted facts and prose are written to `sections/*.tex`, `references.bib`, `figures/`, or `.copilot/handoff.md` only after your coordinator review verifies spec compliance, evidence, and conflicts.
...
## Writer worker patterns

- Fact-extraction workers may map claims to `.copilot/experiments.md`, `.copilot/literature.md`, and existing tex.
- Citation workers may list placeholder citations and needed retrieval follow-up.
- Draft workers may produce a bounded paragraph or subsection from verified facts.
- LaTeX-safety workers may check commands, refs, labels, math, and escaping.
- You still own final prose integration, claim-evidence alignment, and file writes.
