...
```text
.copilot/pipelines/YYYY-MM-DD-S1-copilot-literature-round-N.md
```
...
Worker returns MUST be reduced into the ledger before they influence stage output. Accepted facts are written to `.copilot/literature.md` only after your coordinator review verifies spec compliance, evidence, and conflicts.
...
## Literature worker patterns

- Retrieval workers may search separate keyword clusters or venues.
- Citation workers may verify metadata and BibTeX for bounded paper lists.
- Summary workers may extract method / weakness / distance fields from supplied papers.
- You merge, deduplicate, and mark uncertainty before writing `.copilot/literature.md`.
