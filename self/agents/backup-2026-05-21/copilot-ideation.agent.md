...
```text
.copilot/pipelines/YYYY-MM-DD-S2-copilot-ideation-round-N.md
```
...
Worker returns MUST be reduced into the ledger before they influence stage output. Accepted facts are written to `.copilot/ideas.md` only after your coordinator review verifies spec compliance, evidence, and conflicts.
...
## Ideation worker patterns

- Prior-work workers may check whether a candidate is already covered by nearby papers.
- Code-scan workers may inspect baseline modules and data interfaces for feasibility.
- Terminology workers may check for collisions against `.copilot/glossary.md`, literature notes, and existing tex.
- You still own candidate synthesis, 5-axis filtering, ranking, and selected-direction wording.
